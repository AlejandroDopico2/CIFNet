...
def graficar_resultados(resultados, nombres_carpetas):
    ...
    x = np.arange(
        len(tareas) + 1
    )  # la ubicación de las etiquetas, +1 para los promedios
    width = (
        0.8 / num_resultados
    )  # el ancho de las barras, ajustado al número de resultados
    ...
    for i, (resultado, nombre_carpeta) in enumerate(zip(resultados, nombres_carpetas)):
        valores_tareas = [
            resultado[tarea][-1] for tarea in tareas
        ]  # Toma el último valor de cada tarea
        valores_con_promedio = valores_tareas + [
            promedios[i]
        ]  # Añadir el promedio al final de las tareas

        bars = ax.bar(
            x - width * (num_resultados / 2 - i),
            valores_con_promedio,
            width,
...
def main():
    parser = argparse.ArgumentParser(
        description="Comparar resultados de múltiples archivos JSON"
    )
    ...
    nombres_carpetas = [
        obtener_nombre_carpeta(json_file) for json_file in args.json_files
    ]
