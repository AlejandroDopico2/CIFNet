...
def analyze_best_hyperparameters(df, output_dir):
    ...
    for dataset in df["dataset"].unique():
        ...
        dataset_df["train_test_ratio"] = (
            dataset_df["train_accuracy"] / dataset_df["test_accuracy"]
        )
        best_train_test_ratio_row = dataset_df.loc[
            dataset_df["train_test_ratio"].idxmin()
        ]
    ...
    print(
        f"Best hyperparameters saved to: {os.path.join(output_dir, 'best_hyperparameters.json')}"
    )
...
def plot_learning_rate_vs_accuracy(df, output_dir):
    ...
    for dataset in df["dataset"].unique():
        dataset_df = df[df["dataset"] == dataset].copy()
        plt.scatter(
            dataset_df["learning_rate"], dataset_df["test_accuracy"], label=dataset
        )
    plt.xscale("log")
