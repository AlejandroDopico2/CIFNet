...
def run_multiple_mains(buffer_sizes, num_runs=2):
    ...
    for buffer_size in buffer_sizes:
        buffer_results = []
        logger.info(
            f"Starting tests with buffer_size {buffer_size}"
        )
        ...
        logger.debug(
            f"Results for buffer_size {buffer_size}: {mean_results}"
        )
...
def plot_results(results):
    ...
    for i, metric in enumerate(metrics):
        ...
        for buffer_size in buffer_sizes:
            ...
            filtered_results = [r for r in results if r[0]["buffer_size"] == buffer_size]
            
            if filtered_results:
        
                mean_value = sum(r[0][metric] for r in filtered_results) / len(filtered_results)
                std_value = (sum((r[0][metric] - mean_value) ** 2 for r in filtered_results) / len(filtered_results)) ** 0.5
            else:
                mean_value = 0
                std_value = 0
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("experiments", "experiments_10-07", "buffer_evaluation", "buffer_size_results_comparison.png"))
