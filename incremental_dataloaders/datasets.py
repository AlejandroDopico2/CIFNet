...
def get_clip_transforms(train: bool) -> transforms.Compose:
    if train:
...
class MNISTDataset(BaseDataset):
    def __init__(
        ...
        super().__init__(root, train, img_size=img_size, use_vit=use_vit, use_clip=use_clip)
...
class CIFAR10Dataset(BaseDataset):
    ...
    def get_transform(self, img_size: Union[Tuple, int]):
        if getattr(self, "use_clip", False):
            return get_clip_transforms(self.train)
