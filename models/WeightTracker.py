...
class WeightTracker:
    ...
    def plot_weight_changes(self):
        ...
        fig, axes = plt.subplots(
            num_classes, 1, figsize=(10, 5 * num_classes), sharex=True
        )
        if num_classes == 1:
            axes = [axes]

        for class_idx, weights_history in tqdm(
            self.weight_history.items(), desc="Classes", unit="class"
        ):
            ax = axes[class_idx]
            weight_changes = torch.stack(weights_history)

            for i in tqdm(
                range(weight_changes.shape[1]),
                desc=f"Class {class_idx} Weights",
                unit="weight",
            ):  # For each weight
    ...
    def plot_weight_statistics(self):
        ...
        for class_idx in tqdm(range(num_classes), desc="Classes", unit="class"):
            ...
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(mean_weights))),
                    y=mean_weights,
                    mode="lines",
                    name=f"Class {class_idx} mean",
                    visible=False,
                )
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(std_weights))),
                    y=std_weights,
                    mode="lines",
                    name=f"Class {class_idx} Std Dev",
                    visible=False,
                )
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=list(range(len(frob_norm))),
                    y=frob_norm,
                    mode="lines",
                    name=f"Class {class_idx} Frobenius Norm",
                    visible=False,
                )
            )
