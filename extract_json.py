...
def parse_log_file(log_content):
    ...
    result['rolann_lambda'] = float(re.search(r'ROLANN Lambda: ([\d.]+)', log_content).group(1))
    ...
    if test_accuracies:
        result['test_accuracy'] = float(test_accuracies[-1])  # Tomar la última precisión de prueba

    # Extraer la precisión final de entrenamiento
    result['train_accuracy'] = float(re.search(r'Train Accuracy: ([\d.]+)', log_content).group(1))
